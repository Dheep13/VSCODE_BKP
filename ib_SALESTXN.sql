...
--	v_tenantid = :v_puRow.TENANTID;
	...
	*/
	...
	UPDATE
		pa
	SET
		pa.positionname=pos2.name
	from ext.TRYG_IB_SALESTXNS_POLICYASSIGNS pa
	join cs_stagesalestransaction sst on sst.batchname=pa.batchname and sst.orderid=pa.orderid and sst.linenumber=pa.linenumber and sst.sublinenumber=pa.sublinenumber
	join cs_period per on per.startdate<= sst.compensationdate and per.enddate> sst.compensationdate and per.removedate=to_date('01012200','mmddyyyy') and periodtypeseq=2814749767106561
	join cs_position pos on pos.name=pa.positionname and pos.removedate=v_removeDate AND pos.EFFECTIVEENDDATE > per.STARTDATE AND pos.EFFECTIVESTARTDATE <= per.endDATE
	...
	join cs_position pos2 on pos2.payeeseq=pay.payeeseq and pos2.removedate=v_removeDate AND pos2.EFFECTIVEENDDATE > per.STARTDATE 
	AND pos2.EFFECTIVESTARTDATE <= per.endDATE and pos2.effectivestartdate>=v_changeDate
	where pos.name <> pos2.name
	and pa.batchname = in_batchName;
