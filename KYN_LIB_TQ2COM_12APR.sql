...
BEGIN 
  begin autonomous transaction
    update ext.kyn_tq2com_sync set message = (i_text ||' . '|| i_value), process_flag=1
    where run_key =:i_runKey ;
    commit;
  end;
END;
...
-- rowcount = record_count(:ex_employees);
  ...
  PUBLIC procedure get_tq_from_program( in i_runKey bigint )
   LANGUAGE SQLSCRIPT
   SQL SECURITY INVOKER
   AS
begin
DECLARE v_filterCount number;
DECLARE v_filterTerr number;
DECLARE EXIT HANDLER FOR SQLEXCEPTION
begin
 			EXT.kyn_prc_debug (::SQL_ERROR_CODE, ::SQL_ERROR_MESSAGE);
      log_error_to_sync (::SQL_ERROR_CODE, ::SQL_ERROR_MESSAGE, :i_runKey);
...
update ext.kyn_tq2com_sync s set (
territoryprogram_esd,
territoryprogram_eed,
territoryprogram_periodtype,
territoryprogram_calendar,
territoryprogram_periodseq,
territoryprogram_period) = (select 
cast(tp.effectivestartdate as date) as esd,
cast(tp.effectiveenddate as date) as eed,
pt.name as periodtype,
cal_tp.name as calendar,
...
and s.process_flag=0)
where s.process_flag=0;

v_sqlCount = ::ROWCOUNT;	
CALL EXT.kyn_prc_debug(:v_procName||'=> For run_key '|| :i_runKey ||' kyn_tq2com_sync table updated territory program effectivedates information',v_sqlCount);	
...
v_sqlCount = ::ROWCOUNT;	
CALL EXT.kyn_prc_debug(:v_procName||'=> kyn_tq2com_sync table updated with semi annual period information',v_sqlCount);	


select count(1) into v_filterCount from ext.kyn_tq2com_filter
where run_key =:i_runKey;

select count(1) into v_filterTerr from ext.kyn_tq2com_filter
where run_key =:i_runKey and filter_column='TERRITORY';
...
and jc.run_key=:i_runKey
);

elseif v_filterCount > 0 and v_filterTerr = 0 then
...
join ext.kyn_tq2com_filter fil on 
  jc.run_key=fil.run_key
  and pos.name = fil.filter_value 
...
and jc.run_key=:i_runKey
and exists (select * 
from EXT.KYN_V_TQ2Com_WF_Territory tv where tv.territory_path like '%'||filter_value||'%'
	and tv.territory_name=t.name
)
);
...
END;
  PUBLIC procedure load_quota_to_prestage( in i_runKey bigint)
   LANGUAGE SQLSCRIPT
   SQL SECURITY INVOKER
   AS
begin
...
where run_key in (select run_key from ext.kyn_tq2com_sync where process_flag = 0 and run_key=:i_runKey);
...
COMMIT;
  ...
  PUBLIC procedure get_account_from_tq(in i_runKey bigint)
   LANGUAGE SQLSCRIPT
   SQL SECURITY INVOKER
   AS
begin
...
CALL EXT.kyn_prc_debug(:v_procName||'=> kyn_tq2com_account table load with accounts information complete',v_sqlCount);	
  ...
  PUBLIC procedure get_product_from_tq(in i_runKey bigint )
   LANGUAGE SQLSCRIPT
   SQL SECURITY INVOKER
   AS
begin
...
--'TQ2COM_'||TO_CHAR(current_timestamp,'YYYYMMDD_HH24MISS') 
:i_batchname as batchname, -- need to set the batchname
...
COMMIT;
  ...
  PUBLIC procedure set_active_status_quota( )
   LANGUAGE SQLSCRIPT
   SQL SECURITY INVOKER
   AS
begin
...
v_sqlCount = ::ROWCOUNT;	
CALL EXT.kyn_prc_debug(:v_procName||'=> kyn_tq2com_prestage_quota table - Older version of quotas end dated',:v_sqlCount);	


end;
  PUBLIC procedure set_active_status_account( )
   LANGUAGE SQLSCRIPT
   SQL SECURITY INVOKER
   AS
begin
...
v_sqlCount = ::ROWCOUNT;	
CALL EXT.kyn_prc_debug(:v_procName||'=> kyn_tq2com_account table - Older version of accounts end dated ',:v_sqlCount);	

end;
  PUBLIC procedure set_active_status_product( )
   LANGUAGE SQLSCRIPT
   SQL SECURITY INVOKER
   AS
begin
...
v_sqlCount = ::ROWCOUNT;	
CALL EXT.kyn_prc_debug(:v_procName||'=> kyn_tq2com_product table - Older version of products end dated',:v_sqlCount);	

COMMIT;

end;
  PUBLIC procedure trigger_quota_import(i_batchname varchar(90) )
   LANGUAGE SQLSCRIPT
   SQL SECURITY INVOKER
   AS
begin
...
DECLARE EXIT HANDLER FOR SQLEXCEPTION
      ...
      if :v_count > 0 then
        ...
        EXT.kyn_prc_debug ('Quota import triggered for batchname => '|| :i_batchname ||' with Pipelinerunseq => '|| :v_pipelinerunseq);
      end if;
/*
...
*/
  ...
  PUBLIC procedure update_sync(in i_runKey bigint) as
  begin
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    	BEGIN
 			EXT.kyn_prc_debug (::SQL_ERROR_CODE, ::SQL_ERROR_MESSAGE);
  ...
  update ext.kyn_tq2com_sync set process_flag = 3 where process_flag = 0 and run_key=:i_runKey;
  end;
  ...
  declare v_max_run_key BIGINT;
  declare v_batchname varchar(90) := 'TQ2COM_'||TO_CHAR(current_timestamp,'YYYYMMDD_HH24MISS');
  declare cursor c_key for SELECT run_key
  FROM ext.kyn_tq2com_sync where process_flag=0;
/*
  ...
  */
  ...
  create_lock(:v_process_name, v_lock_success);
...
for x as c_key
  do
  -- loop start for run_key in sync where process_flag = 0 - if error then catch and set process_flag = 1 and message = sql_error+msg in sync table
  get_tq_from_program(x.run_key); -- add run_key input to this and the other procs in this loop
  load_quota_to_prestage(x.run_key);
  -- get_account_and_product_from_tq(); -- split this into 2 - one for account and another for product
  ...
  -- set_active_status(); -- split into 3 one account, product, quota
  set_active_status_quota();
